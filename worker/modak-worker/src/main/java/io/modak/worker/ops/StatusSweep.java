    ...
    private void lakeStats(Statement s, Map<Long, String> names) throws Exception {
        try (ResultSet rs = s.executeQuery(
                "SELECT table_id, kv.key, kv.value::numeric "
                        + "FROM modak.lake_stats, jsonb_each_text(stats) kv")) {
            while (rs.next()) {
                String name = names.get(rs.getLong(1));
                if (name == null) {
                    continue;
                ...
                metrics.gauge(Metrics.series("modak_lake_" + key, "table", name), value);
                series.record("lake_" + key + "|" + name, value);
            }
        }
        try (ResultSet rs = s.executeQuery(
                "SELECT table_id, jsonb_array_length(warnings) FROM modak.lake_stats")) {
            while (rs.next()) {
                String name = names.get(rs.getLong(1));
                if (name != null) {
                    metrics.gauge(Metrics.series("modak_lake_warnings", "table", name),
                            rs.getLong(2));
                }
            }
        }
