...
/** The offboarding command ({@code modak-worker unregister}). */
    ...
    private static void dropColdMirrorTriggers(Connection admin, long tableOid) {
        try (Statement s = admin.createStatement()) {
            try (ResultSet rs = s.executeQuery(
                    "SELECT to_regprocedure('modak_detach_cold_mirror(oid)') IS NOT NULL")) {
                if (!rs.next() || !rs.getBoolean(1)) {
                    return;
                }
            }
            try (ResultSet rs = s.executeQuery(
                    "SELECT modak_detach_cold_mirror(" + tableOid + "::oid)")) {
                if (rs.next()) {
