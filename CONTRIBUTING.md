...
That brings up Postgres with the extension, MinIO, and the worker, then runs
the scripted walkthrough. The console lives at http://localhost:9090.
...
- Run the tests above plus `./example/run.sh` against a fresh stack, since the
  example doubles as the end-to-end suite.
- Follow the surrounding code style, and keep comments and docs plain and
  concise.
