...
//! a row trigger that mirrors every change into `modak.delta`, so plain DML
...
fn modak_cold_mirror_route(
    ...
    match op {
        ...
        other => error!("modak: unknown cold mirror op '{other}'"),
    }
}
...
CREATE FUNCTION modak.cold_mirror() RETURNS trigger
LANGUAGE plpgsql AS $body$
BEGIN
    IF TG_OP = 'DELETE' THEN
        PERFORM modak_cold_mirror_route(TG_ARGV[0]::oid, TG_OP, to_jsonb(OLD), NULL::jsonb);
    ELSIF TG_OP = 'UPDATE' THEN
        PERFORM modak_cold_mirror_route(TG_ARGV[0]::oid, TG_OP, to_jsonb(NEW), to_jsonb(OLD));
    ELSE
        PERFORM modak_cold_mirror_route(TG_ARGV[0]::oid, TG_OP, to_jsonb(NEW), NULL::jsonb);
    END IF;
    RETURN NULL;
END
$body$;
"#,
    ...
    requires = [modak_cold_mirror_route]
);

pub(crate) const TRIGGER_NAME: &str = "modak_cold_mirror";

#[pg_extern]
fn modak_attach_cold_mirror(table: pg_sys::Oid, partition: pg_sys::Oid) -> String {
    let t = TableId(table.into());
    let meta = or_error(write_meta(t));
    if !meta.keep_heap {
        error!(
            "modak: {}.{} does not keep its heap, the cold mirror applies \
    ...
    or_error(
        Spi::run(&format!(
            "CREATE TRIGGER {TRIGGER_NAME} AFTER INSERT OR UPDATE OR DELETE ON {qualified} \
             FOR EACH ROW EXECUTE FUNCTION modak.cold_mirror('{}')",
            t.0
        ))
        .map_err(catalog_err),
    );
...
#[pg_extern]
fn modak_detach_cold_mirror(table: pg_sys::Oid) -> i64 {
    let children: Vec<String> = Spi::connect(|client| {
        let rows = client
            ...
            .unwrap_or_else(|e| error!("modak: {e}"));
