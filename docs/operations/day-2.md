...
The cold tier needs upkeep (small-file compaction, delete-debt compaction,
snapshot expiry, metadata hygiene) and Modak owns it by default, with per-pass
and per-table opt-outs for teams that already run their own. It is a topic of
its own: see [Lake maintenance](lake-maintenance.md), including what must stay
with Modak (anything that deletes files, snapshot expiry above all) and what
you can safely run externally.
