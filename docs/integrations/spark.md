...
For the same reason, never run Spark's `expire_snapshots` or
`remove_orphan_files` procedures against a Modak-managed table, they cannot
see the pins. See [Lake maintenance](../operations/lake-maintenance.md) for
what Spark may safely maintain.
...
[hybrid mode](../tables/reading.md#mirrored-tables-heap-or-hybrid):
...
belong on the [bulk ingestion](../ingestion/bulk-ingestion.md) path, which commits
