...
DML. A keep-heap table's heap also holds everything, so its historical writes
are plain heap DML too: the cold-mirror trigger on tiered partitions carries
each change into `modak.delta`, and lake-side paths (`ingest`, historical
Stream Load rows) should be avoided because they bypass the heap copy.
