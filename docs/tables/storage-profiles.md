...
| `--format` | Lake format plugin id. Omit to use the worker's `MODAK_LAKE_FORMAT` |
| `--config` | Semicolon-separated `key=value` overrides, same keys as `MODAK_LAKE_PROPS`. A blank value (`key=`) removes an inherited default |
...
```bash
# S3-compatible
MODAK_CREDENTIALS_ANALYTICS='s3.access-key=AKIA...;s3.secret-key=...;s3.region=us-east-2'
